...
public class BidirectionalGroupMap {
	...
	/*
	 * public String toLatexGraph() { // Helper to escape underscores in displayed
	 * labels java.util.function.Function<String, String> escapeLabel = s ->
	 * s.replace("_", "\\_");
	 * 
	 * // Helper to remove underscores from internal TikZ node names
	 * java.util.function.Function<String, String> removeUnderscores = s ->
	 * s.replace("_", "");
	 * 
	 * // We'll define fixed spacing for group nodes double groupSpacing = -2;
	 * 
	 * // We'll define smaller spacing for headers, so they cluster near each group
	 * double headerSpacing = 1;
	 * 
	 * // 1) Gather / sort reference group names java.util.List<String> refNames =
	 * new java.util.ArrayList<>(refToGroups.keySet());
	 * java.util.Collections.sort(refNames);
	 * 
	 * // 2) Gather / sort target group names java.util.List<String> tgtNames = new
	 * java.util.ArrayList<>(targetToGroups.keySet());
	 * java.util.Collections.sort(tgtNames);
	 * 
	 * // ----------------------------------------------------------------- // We'll
	 * keep track of node IDs so we can create "fit" background boxes: // -
	 * refHeaderIDs: All reference header node IDs // - groupIDs: All
	 * reference-group and target-group node IDs // - tgtHeaderIDs: All target
	 * header node IDs //
	 * -----------------------------------------------------------------
	 * java.util.List<String> refHeaderIDs = new java.util.ArrayList<>();
	 * java.util.List<String> groupIDs = new java.util.ArrayList<>();
	 * java.util.List<String> tgtHeaderIDs = new java.util.ArrayList<>();
	 * 
	 * // 3) Build up the LaTeX StringBuilder sb = new StringBuilder();
	 * sb.append("\\documentclass[tikz]{standalone}\n").append(
	 * "\\usepackage{tikz}\n") // we need fit and backgrounds libraries for
	 * background boxes:
	 * .append("\\usetikzlibrary{arrows.meta,positioning,fit,backgrounds}\n").append
	 * ("\\begin{document}\n") .append("\\begin{tikzpicture}[\n").
	 * append("    xscale=2,        % <-- Double the width\n")
	 * .append("    >={Stealth},     % arrow tip\n").
	 * append("    auto,            % automatic edge labels\n")
	 * .append("    node distance=1.6cm,\n").
	 * append("    semithick        % line style\n").append("]\n\n");
	 * 
	 * // ------------------------------------------------------ // (A) Place
	 * Reference Groups (x=0) and their Headers (x=-2) //
	 * ------------------------------------------------------ for (int i = 0; i <
	 * refNames.size(); i++) { String groupName = refNames.get(i); // Typically each
	 * group name has exactly one GroupBlock in refToGroups:
	 * java.util.List<GroupBlock> blocks = refToGroups.get(groupName); if (blocks ==
	 * null) continue;
	 * 
	 * for (int bIndex = 0; bIndex < blocks.size(); bIndex++) { GroupBlock block =
	 * blocks.get(bIndex);
	 * 
	 * String nodeID = "ref" + removeUnderscores.apply(block.groupName); String
	 * nodeLbl = escapeLabel.apply(block.groupName);
	 * 
	 * double groupY = i * groupSpacing + (bIndex * (groupSpacing / 2.0));
	 * 
	 * // Place the reference group node at (0, groupY)
	 * sb.append(String.format("\\node (%s) at (0, %.2f) {%s};\n", nodeID, groupY,
	 * nodeLbl));
	 * 
	 * // This group node belongs in the "groupIDs" (middle box)
	 * groupIDs.add(nodeID);
	 * 
	 * // Now draw the headers for this group block, at x = -2 int hIndex = 0; for
	 * (String header : block.getHeaders()) { // Create a unique node ID for each
	 * header String headerID = "hdrRef" + removeUnderscores.apply(block.groupName)
	 * + "_" + removeUnderscores.apply(header);
	 * 
	 * // Escape underscores in the label String headerLbl =
	 * escapeLabel.apply(header);
	 * 
	 * double headerY = groupY - (hIndex * headerSpacing);
	 * 
	 * // Place the header node
	 * sb.append(String.format("\\node (%s) at (-2, %.2f) {%s};\n", headerID,
	 * headerY, headerLbl));
	 * 
	 * // This header node belongs in the "refHeaderIDs" (left box)
	 * refHeaderIDs.add(headerID);
	 * 
	 * // Retrieve the (already-softmaxed) weight from the block Double w =
	 * block.getHeaderWeight(header); String wLabel = String.format("%.2f", w); //
	 * e.g. "0.25"
	 * 
	 * // Draw arrow: header -> group, labeling with weight // "midway, above" =>
	 * label is placed halfway along the arrow, above it sb.append(String.
	 * format("\\draw[->] (%s) -- node[midway,above,sloped,black]{%s} (%s);\n",
	 * headerID, wLabel, nodeID));
	 * 
	 * hIndex++; } sb.append("\n"); } }
	 * 
	 * // ------------------------------------------------------ // (B) Place Target
	 * Groups (x=4) and their Headers (x=6) //
	 * ------------------------------------------------------ for (int i = 0; i <
	 * tgtNames.size(); i++) { String groupName = tgtNames.get(i);
	 * java.util.List<GroupBlock> blocks = targetToGroups.get(groupName); if (blocks
	 * == null) continue;
	 * 
	 * for (int bIndex = 0; bIndex < blocks.size(); bIndex++) { GroupBlock block =
	 * blocks.get(bIndex);
	 * 
	 * String nodeID = "tgt" + removeUnderscores.apply(block.groupName); String
	 * nodeLbl = escapeLabel.apply(block.groupName);
	 * 
	 * double groupY = i * groupSpacing + (bIndex * (groupSpacing / 2.0));
	 * 
	 * // Place the target group node at (4, groupY)
	 * sb.append(String.format("\\node (%s) at (4, %.2f) {%s};\n", nodeID, groupY,
	 * nodeLbl));
	 * 
	 * // This group node is also part of the "groupIDs" (middle box)
	 * groupIDs.add(nodeID);
	 * 
	 * // Now draw the headers for this block at x=6 int hIndex = 0; for (String
	 * header : block.getHeaders()) { String headerID = "hdrTgt" +
	 * removeUnderscores.apply(block.groupName) + "_" +
	 * removeUnderscores.apply(header); String headerLbl =
	 * escapeLabel.apply(header);
	 * 
	 * double headerY = groupY - (hIndex * headerSpacing);
	 * 
	 ...
	 * headerY, headerLbl));
	 * 
	 * // This node goes to "tgtHeaderIDs" (right box) tgtHeaderIDs.add(headerID);
	 * 
	 * // Get the weight from the block Double w = block.getHeaderWeight(header);
	 * String wLabel = String.format("%.2f", w);
	 * 
	 * // Arrow from header -> target group, with weight label sb.append(String.
	 * format("\\draw[->] (%s) -- node[midway,above,sloped,black]{%s} (%s);\n",
	 * headerID, wLabel, nodeID));
	 * 
	 * hIndex++; } sb.append("\n"); } }
	 * 
	 * // ------------------------------------------------------ // (C) Draw arrows
	 * for refGroupsToTargetGroups //
	 * ------------------------------------------------------
	 * sb.append("\n% -- Ref->Tgt edges --\n"); for
	 * (java.util.Map.Entry<java.util.List<GroupBlock>, java.util.List<GroupBlock>>
	 * entry : refGroupsToTargetGroups .entrySet()) {
	 * 
	 * java.util.List<GroupBlock> refBlocks = entry.getKey();
	 * java.util.List<GroupBlock> tgtBlocks = entry.getValue();
	 * 
	 * for (GroupBlock refBlock : refBlocks) { String refNodeID = "ref" +
	 * removeUnderscores.apply(refBlock.groupName);
	 * 
	 * for (GroupBlock tgtBlock : tgtBlocks) { String tgtNodeID = "tgt" +
	 * removeUnderscores.apply(tgtBlock.groupName);
	 * 
	 * // We do NOT have a single "weight" for group->group edges in your design, //
	 * so we just draw them as before (without labeling).
	 * sb.append(String.format("\\draw[->] (%s) -- (%s);\n", refNodeID, tgtNodeID));
	 * } } }
	 * 
	 * // ------------------------------------------------------ // (D) Draw arrows
	 * for targetGrupsToRefGroups //
	 * ------------------------------------------------------
	 * sb.append("\n% -- Tgt->Ref edges --\n"); for
	 * (java.util.Map.Entry<java.util.List<GroupBlock>, java.util.List<GroupBlock>>
	 * entry : targetGrupsToRefGroups .entrySet()) {
	 * 
	 * java.util.List<GroupBlock> tgtBlocks = entry.getKey();
	 * java.util.List<GroupBlock> refBlocks = entry.getValue();
	 * 
	 * for (GroupBlock tgtBlock : tgtBlocks) { String tgtNodeID = "tgt" +
	 * removeUnderscores.apply(tgtBlock.groupName);
	 * 
	 ...
	 * removeUnderscores.apply(refBlock.groupName);
	 * 
	 * // Similarly, no single "header weight" for group->group edges, so no label
	 * sb.append(String.format("\\draw[->] (%s) -- (%s);\n", tgtNodeID, refNodeID));
	 * } } }
	 * 
	 * // ------------------------------------------------------ // (E) Background
	 * "fit" boxes for the three sections //
	 * ------------------------------------------------------
	 * sb.append("\n% -- Background boxes for the three sections --\n");
	 * 
	 * // 1) Reference Headers box if (!refHeaderIDs.isEmpty()) {
	 * sb.append("\\begin{scope}[on background layer]\n").
	 * append("  \\node[fill=blue!10,rounded corners,")
	 * .append("        label=above:{Reference Headers},").append("        fit=");
	 * sb.append(" (").append(String.join(")(", refHeaderIDs)).append(")");
	 * sb.append("] {};\n\\end{scope}\n\n"); }
	 * 
	 * // 2) Groups box (reference groups + target groups) if (!groupIDs.isEmpty())
	 * { sb.append("\\begin{scope}[on background layer]\n").
	 * append("  \\node[fill=green!10,rounded corners,")
	 * .append("        label=above:{Groups},").append("        fit=");
	 * sb.append(" (").append(String.join(")(", groupIDs)).append(")");
	 * sb.append("] {};\n\\end{scope}\n\n"); }
	 * 
	 * // 3) Target Headers box if (!tgtHeaderIDs.isEmpty()) {
	 * sb.append("\\begin{scope}[on background layer]\n").
	 * append("  \\node[fill=red!10,rounded corners,")
	 * .append("        label=above:{Target Headers},").append("        fit=");
	 * sb.append(" (").append(String.join(")(", tgtHeaderIDs)).append(")");
